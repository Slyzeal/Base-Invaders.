...
// ── localStorage storage (persists scores per wallet) ─────────────────────────
const storage = {
  ...
  list: async (prefix) => { try { const keys = []; for (let i = 0; i < localStorage.length; i++) { const k = localStorage.key(i); if (k && k.startsWith(prefix)) keys.push(k); } return { keys }; } catch { return { keys: [] }; } },
};
...
async function resolveBasename(address) {
  ...
  try {
    const res2 = await fetch("https://cloudflare-eth.com", {
      ...
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "eth_call", params: [{ to: "0x4976fb03C32e5B8cfe2b6cCB31c09Ba78EBaBa41", data: `0x55ea6c47000000000000000000000000${key.slice(2)}` }, "latest"] })
    });
...
async function getBest(w) {
  try { const r = await window.storage.get(`bi6:${w.toLowerCase()}`); return r ? +r.value : 0; } catch { return 0; }
}
async function saveBest(w, s, basename) {
  try {
    ...
    if (s > b) {
      await window.storage.set(`bi6:${key}`, String(s));
      await window.storage.set(`lb6:${key}`, JSON.stringify({ wallet: w, score: s, basename: basename || null }));
    }
  } catch {}
}
async function getLeaderboard() {
  try {
    const res = await window.storage.list("lb6:");
    if (!res?.keys?.length) return [];
    const rows = await Promise.all(res.keys.map(async k => {
      try { const r = await window.storage.get(k); return r ? JSON.parse(r.value) : null; } catch { return null; }
    }));
    return rows.filter(Boolean).sort((a, b) => b.score - a.score).slice(0, 10);
  } catch { return []; }
}
...
export default function App() {
  ...
  const connect = async () => {
    setWalletError("");
    if (!window.ethereum) { setWalletError("No wallet found. Open in Coinbase Wallet or install MetaMask."); return; }
    setConnecting(true);
    try {
      ...
      setWallet(addr); setBasename(bn || ""); getBest(addr).then(setBest);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const handler = (accounts) => {
      if (accounts[0]) { const addr = accounts[0]; resolveBasename(addr).then(bn => setBasename(bn || "")); setWallet(addr); getBest(addr).then(setBest); }
    ...
    <div className="sc" style={{ position: "relative", width: W, borderRadius: 22, overflow: "hidden", boxShadow: "0 0 100px rgba(0,50,190,.32),0 0 0 1px rgba(0,80,220,.15)" }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", padding: "26px 26px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 4 }}>
          <div style={{ fontSize: 9, letterSpacing: 8, color: "rgba(0,150,255,.5)", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, marginBottom: 8 }}>STAR  BASE  NETWORK</div>
        ...
        <div style={{ background: "rgba(0,10,40,.75)", backdropFilter: "blur(16px)", border: "1px solid rgba(0,80,200,.2)", borderRadius: 16, padding: "14px 18px", marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 12px" }}>
            {[["DRAG / ARROWS", "Move ship"], ["AUTO-FIRE", "No trigger needed"], ["BLUE = $BASE", "x2 score (rare)"], ["CYAN = FREEZE", "Stops enemies"], ["RED = LASER", "Dbl-tap to fire"], ["x10 KILLS", "Next wave"]].map(([k, v]) => (
        ...
        {!wallet ? (
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 8, letterSpacing: 2, color: "rgba(100,150,255,.45)", marginBottom: 10, textAlign: "center", fontFamily: "'Rajdhani',sans-serif", lineHeight: 1.8 }}>
              CONNECT YOUR BASE WALLET TO SAVE SCORES<br />
              <span style={{ color: "rgba(80,120,200,.35)", fontSize: 7 }}>YOUR ACTUAL WALLET SIGNS IN — NO PASSWORD — NO ACCOUNT</span>
            </div>
            ...
            {walletError && (
              <div style={{ marginTop: 10, fontSize: 11, color: "#ff7766", textAlign: "center", fontFamily: "'Exo 2',sans-serif", lineHeight: 1.5, background: "rgba(255,50,30,.08)", border: "1px solid rgba(255,80,50,.15)", borderRadius: 10, padding: "8px 12px" }}>{walletError}</div>
            )}
    ...
    <div className="sc" style={{ width: W, background: "rgba(1,1,16,.98)", border: "1px solid rgba(0,60,180,.2)", borderRadius: 22, overflow: "hidden", boxShadow: "0 0 100px rgba(0,30,140,.42)" }}>
      <div style={{ padding: "36px 28px" }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ marginTop: 8, fontSize: 8, color: "rgba(120,160,200,.4)", fontFamily: "'Exo 2',sans-serif", letterSpacing: 2 }}>Ranked by score · Basenames shown</div>
        </div>
        {lb.length === 0 ? (
            ...
            {lb.map((e, i) => {
              ...
              return (
                <div key={e.wallet} style={{ display: "flex", alignItems: "center", gap: 12, background: isMe ? "rgba(0,50,165,.25)" : (rankBg[i] || "rgba(0,14,45,.5)"), border: `1px solid ${isMe ? "rgba(0,120,255,.42)" : i === 0 ? "rgba(255,200,0,.22)" : i === 1 ? "rgba(200,200,200,.12)" : i === 2 ? "rgba(180,120,50,.12)" : "rgba(0,50,140,.15)"}`, borderRadius: 12, padding: "13px 15px" }}>
                  ...
                  <div style={{ color: i === 0 ? "#ffd700" : i === 1 ? "#d0d0d0" : i === 2 ? "#cd8f4a" : isMe ? "#7fc4ff" : "rgba(220,235,255,.88)", fontWeight: 900, fontSize: 19, fontFamily: "'Rajdhani',sans-serif", letterSpacing: .5, flexShrink: 0 }}>{e.score.toLocaleString()}</div>
                </div>
              );
            })}
          </div>
