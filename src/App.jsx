...
// Farcaster Mini App SDK — auto-detects wallet when running inside Warpcast / Coinbase Wallet
let sdk = null;
try { sdk = require("@farcaster/frame-sdk").default; } catch {}

// ── localStorage-backed storage (replaces artifact window.storage) ────────────
const storage = {
  get: async (key) => {
...
// Make it available globally so the game functions work unchanged
...
/* ═══════════════════════════════════════════════════════════
   BASE DOMAIN LOOKUP  (Basenames — no sign-in, read-only)
   Resolves <address>.base.eth via public ENS over Basenames
═══════════════════════════════════════════════════════════ */
const domainCache = {};
async function resolveBasename(address) {
  ...
  try {
    // Basenames are ERC-137 names on Base mainnet.
    // We query the public Basenames API which resolves address → name.
    const res = await fetch(
      `https://api.basename.app/v1/names?address=${key}`,
  ...
  // Fallback: try ENS reverse resolution via public RPC
  try {
    const res2 = await fetch("https://cloudflare-eth.com", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        jsonrpc: "2.0", id: 1, method: "eth_call",
        params: [{ to: "0x4976fb03C32e5B8cfe2b6cCB31c09Ba78EBaBa41", data: `0x55ea6c47000000000000000000000000${key.slice(2)}` }, "latest"]
      })
    });
    const json2 = await res2.json();
    if (json2?.result && json2.result !== "0x") {
      // decode ABI string — very basic
      const hex = json2.result.slice(2);
      const offset = parseInt(hex.slice(0, 64), 16) * 2;
      const len = parseInt(hex.slice(offset, offset + 64), 16);
      const nameHex = hex.slice(offset + 64, offset + 64 + len * 2);
      let name = "";
      for (let i = 0; i < nameHex.length; i += 2) name += String.fromCharCode(parseInt(nameHex.slice(i, i + 2), 16));
      if (name) { domainCache[key] = name; return name; }
    }
  } catch {}
  domainCache[key] = null;
  return null;
}

function displayName(wallet, basename) {
  if (basename) return basename;
  return shortAddr(wallet);
}

/* ═══════════════════════════════════════════════════════════
   STORAGE  (wallet-keyed scores only, no auth)
═══════════════════════════════════════════════════════════ */
async function getBest(w) {
  try { const r = await window.storage.get(`bi5:${w.toLowerCase()}`); return r ? +r.value : 0; } catch { return 0; }
}
async function saveBest(w, s, basename) {
  try {
    const key = w.toLowerCase();
    const b = await getBest(key);
    if (s > b) {
      await window.storage.set(`bi5:${key}`, String(s));
      await window.storage.set(`lb5:${key}`, JSON.stringify({ wallet: w, score: s, basename: basename || null }), true);
    }
  } catch {}
}
async function getLeaderboard() {
  try {
    const res = await window.storage.list("lb5:", true);
...
class AudioEngine {
  constructor() {
    ...
    this.musicEnabled = true; this._mt = null; this._at = null; this._dt = null;
  }
  init() {
    if (this.ctx) return;
  ...
  _note(freq, start, dur, gain, type, dest) {
    if (!this.ctx) return;
    ...
    o.connect(f); f.connect(e); e.connect(dest || this.musicGain); o.start(start); o.stop(start + dur + 0.05); this.oscillators.push(o);
  }
  _bass() {
  ...
  laserStart() {
    ...
    e.gain.setValueAtTime(0,now); e.gain.linearRampToValueAtTime(0.4,now+0.05); e.gain.linearRampToValueAtTime(0.2,now+1);
    e.gain.linearRampToValueAtTime(0.001,now+10);
    o.connect(f); f.connect(e); e.connect(this.sfxGain); o.start(now); o.stop(now+10.1);
    this._laserOsc=o;
  }
...
/* ═══════════════════════════════════════════════════════════
   ALIEN PALETTES  [bodyHi, bodyMid, bodyLo, eyeCol, glowRGB, accentRGB]
═══════════════════════════════════════════════════════════ */
const PALETTES=[
...
/* ═══════════════════════════════════════════════════════════
   DRAW: ALIEN  — unified shape, palette per wave
═══════════════════════════════════════════════════════════ */
function drawAlien(ctx,x,y,phase,frozen,wave){
  ...
  const bob=Math.sin(phase*2.2)*2; ctx.translate(0,bob);
  // halo
  ...
  // tentacles
  ctx.strokeStyle=c1; ctx.lineWidth=2.8; ctx.lineCap="round";
  [[-10,14,-22,26,-18,36,0],[10,14,22,26,18,36,1],[0,16,0,28,0,38,2]].forEach(([sx,sy,cx2,cy2,ex2,ey2,idx])=>{
    ctx.strokeStyle=idx===2?c0:c1; ctx.lineWidth=idx===2?2.2:2.8;
  ...
  // freeze
  if(frozen){ctx.save();ctx.globalAlpha=.32;ctx.fillStyle="#aaeeff";ctx.beginPath();ctx.arc(0,0,20,0,Math.PI*2);ctx.fill();ctx.restore();ctx.save();ctx.strokeStyle="rgba(200,255,255,.6)";ctx.lineWidth=1;for(let i=0;i<4;i++){ctx.save();ctx.rotate((Math.PI/2)*i+phase*.3);ctx.beginPath();ctx.moveTo(0,-14);ctx.lineTo(0,-22);ctx.stroke();ctx.restore();}ctx.restore();}
  ctx.restore();
}
...
function drawShip(ctx,x,y,mult,frozen,laser,phase,tilt=0){
  ...
  // fc0 unused — colours handled inline below
  // center flame
  ...
  cf.addColorStop(.4,laser?"rgba(200,0,0,0.85)":frozen?"rgba(0,180,160,0.85)":mult?"rgba(0,85,255,0.85)":"rgba(255,100,10,0.85)");
  ...
  ctx.fillStyle=cg; rRect(ctx,SHIP_W*.5-3.5,-11,7,12,2); ctx.fill();
  if(laser||mult||frozen){ctx.shadowColor=laser?"#ff4400":mult?"#00ddff":"#00ffcc"; ctx.shadowBlur=16; ctx.beginPath(); ctx.arc(SHIP_W*.5,-11,3.2,0,Math.PI*2); ctx.fillStyle=laser?"#ff8866":mult?"#aaeeff":"#aaffee"; ctx.fill(); ctx.shadowBlur=0;}
  // cockpit
  const ck=ctx.createRadialGradient(SHIP_W*.46,SHIP_H*.20,1,SHIP_W*.5,SHIP_H*.27,11);
  ck.addColorStop(0,"#eef8ff"); ck.addColorStop(.5,laser?"#ff8844":frozen?"#44ffdd":mult?"#44ccff":"#66aaee"); ck.addColorStop(1,"rgba(0,18,80,0.9)");
  ...
  if(laser||mult||frozen){ctx.save();ctx.globalAlpha=.09+Math.sin(phase*4)*.05;const au=ctx.createRadialGradient(SHIP_W*.5,SHIP_H*.5,0,SHIP_W*.5,SHIP_H*.5,SHIP_W*1.1);au.addColorStop(0,laser?"#ff4400":mult?"#00ccff":frozen?"#00ffdd":"transparent");au.addColorStop(1,"transparent");ctx.fillStyle=au;ctx.fillRect(-SHIP_W*.7,-12,SHIP_W*2.4,SHIP_H+24);ctx.restore();}
  ctx.restore();
}
...
function drawLaserBeam(ctx, shipX, shipY, phase) {
  ...
  const gOuter = ctx.createLinearGradient(cx - 10, 0, cx + 10, 0);
  gOuter.addColorStop(0, "transparent"); gOuter.addColorStop(.3, "rgba(255,60,0,.3)"); gOuter.addColorStop(.5, "rgba(255,120,0,.5)"); gOuter.addColorStop(.7, "rgba(255,60,0,.3)"); gOuter.addColorStop(1, "transparent");
  ctx.fillStyle = gOuter; ctx.fillRect(cx - 10, 0, 20, shipY); ctx.shadowBlur = 0;
  // core beam
  const gCore = ctx.createLinearGradient(cx - 4, 0, cx + 4, 0);
  gCore.addColorStop(0, "rgba(255,200,100,.6)"); gCore.addColorStop(.5, "rgba(255,255,200,1)"); gCore.addColorStop(1, "rgba(255,200,100,.6)");
  ctx.fillStyle = gCore; ctx.fillRect(cx - 4, 0, 8, shipY);
  // bright inner
  ctx.fillStyle = `rgba(255,255,255,${.7+Math.sin(phase*20)*.3})`;
  ctx.fillRect(cx - 1.5, 0, 3, shipY);
  // horizontal scanlines flicker
  ctx.globalAlpha = .12 + Math.sin(phase * 15) * .06;
  ctx.fillStyle = "#ff8800";
  for (let y = 0; y < shipY; y += 8) ctx.fillRect(cx - 6, y, 12, 2);
  ctx.restore();
}

/* ═══════════════════════════════════════════════════════════
   DRAW: LASER FRUIT  (red lightning bolt collectible)
═══════════════════════════════════════════════════════════ */
function drawLaserFruit(ctx, x, y, phase) {
  ctx.save(); ctx.translate(x + 20, y + 20);
  const p = 1 + Math.sin(phase * 2.2) * .06; ctx.scale(p, p);
  // outer glow
  ...
  ctx.beginPath();
  ctx.moveTo(4,-13); ctx.lineTo(-2,-2); ctx.lineTo(3,-2); ctx.lineTo(-4,13); ctx.lineTo(8,0); ctx.lineTo(2,0);
  ctx.closePath(); ctx.fill();
  ctx.shadowBlur = 0;
  // shine
  ctx.save(); ctx.globalAlpha = .28; const sh = ctx.createRadialGradient(-5,-6,1,-4,-5,8); sh.addColorStop(0,"#fff"); sh.addColorStop(1,"transparent"); ctx.fillStyle=sh; ctx.beginPath(); ctx.arc(-4,-5,8,0,Math.PI*2); ctx.fill(); ctx.restore();
  // ring
...
/* ═══════════════════════════════════════════════════════════
   DRAW: BASE FRUIT / FREEZE FRUIT
═══════════════════════════════════════════════════════════ */
function drawBaseFruit(ctx,x,y,phase){
  ctx.save();ctx.translate(x+20,y+20);const p=1+Math.sin(phase*2)*.055;ctx.scale(p,p);
  ctx.save();ctx.globalAlpha=.16+Math.sin(phase)*.08;const og=ctx.createRadialGradient(0,0,6,0,0,28);og.addColorStop(0,"#0066ff");og.addColorStop(1,"transparent");ctx.fillStyle=og;ctx.beginPath();ctx.arc(0,0,28,0,Math.PI*2);ctx.fill();ctx.restore();
  const bg=ctx.createRadialGradient(-5,-5,2,0,0,18);bg.addColorStop(0,"#5ab4ff");bg.addColorStop(.5,"#0055ff");bg.addColorStop(1,"#001880");ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.fillStyle=bg;ctx.fill();
  ctx.fillStyle="#fff";ctx.font="bold 6.5px 'Exo 2',sans-serif";ctx.textAlign="center";ctx.fillText("BASE",0,-3);
  ctx.fillStyle="#88ccff";ctx.font="bold 9px 'Exo 2',sans-serif";ctx.fillText("$",0,8);
  ctx.save();ctx.globalAlpha=.28;const sh=ctx.createRadialGradient(-5,-6,1,-4,-5,9);sh.addColorStop(0,"#fff");sh.addColorStop(1,"transparent");ctx.fillStyle=sh;ctx.beginPath();ctx.arc(-4,-5,9,0,Math.PI*2);ctx.fill();ctx.restore();
  ctx.strokeStyle=`rgba(80,180,255,${.5+Math.sin(phase)*.3})`;ctx.lineWidth=1.8;ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.stroke();ctx.restore();
}
function drawFreezeFruit(ctx,x,y,phase){
  ctx.save();ctx.translate(x+20,y+20);const p=1+Math.sin(phase*2.5)*.07;ctx.scale(p,p);
  ctx.save();ctx.globalAlpha=.2+Math.sin(phase)*.1;const og=ctx.createRadialGradient(0,0,5,0,0,30);og.addColorStop(0,"#00ffee");og.addColorStop(1,"transparent");ctx.fillStyle=og;ctx.beginPath();ctx.arc(0,0,30,0,Math.PI*2);ctx.fill();ctx.restore();
  const bg=ctx.createRadialGradient(-4,-4,2,0,0,18);bg.addColorStop(0,"#aaffee");bg.addColorStop(.45,"#00ccbb");bg.addColorStop(1,"#003344");ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.fillStyle=bg;ctx.fill();
  ...
  ctx.save();ctx.globalAlpha=.3;const sh=ctx.createRadialGradient(-4,-6,1,-4,-5,8);sh.addColorStop(0,"#fff");sh.addColorStop(1,"transparent");ctx.fillStyle=sh;ctx.beginPath();ctx.arc(-4,-5,8,0,Math.PI*2);ctx.fill();ctx.restore();
  ctx.strokeStyle=`rgba(0,255,220,${.55+Math.sin(phase)*.3})`;ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.stroke();ctx.restore();
}

/* ═══════════════════════════════════════════════════════════
   DRAW: PARTICLE
═══════════════════════════════════════════════════════════ */
function drawParticle(ctx,p){ctx.save();ctx.globalAlpha=Math.max(0,p.life);ctx.shadowColor=p.color;ctx.shadowBlur=12;ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.r*Math.max(0,p.life),0,Math.PI*2);ctx.fill();ctx.restore();}
...
function drawBullet(ctx,x,y,mult){
  ...
  ctx.fillStyle=g;rRect(ctx,x-3,y,6,22,3); ctx.fill();
  ctx.fillStyle="#fff";ctx.shadowBlur=24;ctx.shadowColor="#fff";ctx.beginPath();ctx.arc(x,y+2,2.8,0,Math.PI*2);ctx.fill();ctx.restore();
}

/* ═══════════════════════════════════════════════════════════
   DRAW: HUD  — score BIG top-right, lives top-left, wave center
═══════════════════════════════════════════════════════════ */
function drawHUD(ctx, g, now) {
  // ── Top bar ──────────────────────────────────────────────────────────────
  ctx.fillStyle = "rgba(0,3,18,0.86)";
  rRect(ctx,8, 8, W - 16, 56, 12); ctx.fill();
  ctx.strokeStyle = "rgba(0,65,175,0.25)"; ctx.lineWidth = 1;
  rRect(ctx,8, 8, W - 16, 56, 12); ctx.stroke();

  // ── Lives (top-left) ─────────────────────────────────────────────────────
  for (let i = 0; i < 3; i++) {
    const alive = i < g.lives;
    ctx.save(); ctx.globalAlpha = alive ? 1 : 0.14;
    ...
    ctx.fillText("▲", 26+i*26, 40); ctx.restore();
  }

  // ── WAVE badge (centre) ───────────────────────────────────────────────────
  ctx.fillStyle = "rgba(0,70,210,0.2)";
  rRect(ctx,W/2-32,13,64,40,8); ctx.fill();
  ctx.strokeStyle = "rgba(0,110,255,0.22)"; ctx.lineWidth = 1;
  rRect(ctx,W/2-32,13,64,40,8); ctx.stroke();
  ctx.font = "600 8px 'Rajdhani',sans-serif"; ctx.textAlign = "center";
  ctx.fillStyle = "rgba(100,160,255,.55)"; ctx.fillText("WAVE", W/2, 26);
  ctx.font = "800 20px 'Rajdhani',sans-serif"; ctx.fillStyle = "#aaccff"; ctx.fillText(String(g.wave), W/2, 46);

  // ── SCORE (top-right) — BIG & ALWAYS VISIBLE ─────────────────────────────
  // score label
  ctx.font = "700 8px 'Rajdhani',sans-serif"; ctx.textAlign = "right";
  ctx.fillStyle = "rgba(120,175,255,.55)"; ctx.fillText("SCORE", W - 18, 22);
  // score value — large, glowing when multiplied
  const scoreColor = g.mult > 1 ? "#ffd700" : "#ffffff";
  if (g.mult > 1) { ctx.shadowColor = "#ffd700"; ctx.shadowBlur = 20; }
  ctx.font = "900 26px 'Rajdhani',sans-serif"; ctx.textAlign = "right";
  ctx.fillStyle = scoreColor; ctx.fillText(g.score.toLocaleString(), W - 18, 48);
  ctx.shadowBlur = 0;

  // ── Laser charge indicator (below score) ─────────────────────────────────
  if (g.laserStored) {
    ctx.save();
    ctx.font = "700 8px 'Exo 2',sans-serif"; ctx.textAlign = "right";
    ctx.fillStyle = "rgba(255,100,30,.6)"; ctx.fillText("LASER READY", W - 18, 62);
    ctx.shadowColor = "#ff6600"; ctx.shadowBlur = 8;
    ctx.fillStyle = "#ff8833"; ctx.fillText("⚡", W - 80, 62);
    ctx.shadowBlur = 0; ctx.restore();
  }

  // ── Wave progress bar ─────────────────────────────────────────────────────
  const prog = (g.kills % KILLS_PER_WAVE) / KILLS_PER_WAVE;
  ctx.fillStyle = "rgba(0,55,150,0.3)";
  rRect(ctx,8, 68, W-16, 7, 3.5); ctx.fill();
  if (prog > 0) {
    const pg = ctx.createLinearGradient(8,0,8+(W-16)*prog,0);
    pg.addColorStop(0,"#0055ff"); pg.addColorStop(1,"#00ccff");
    ctx.fillStyle = pg; rRect(ctx,8,68,(W-16)*prog,7,3.5); ctx.fill();
  }
  ctx.font="600 7px 'Exo 2'";ctx.textAlign="right";ctx.fillStyle="rgba(100,160,255,.4)";
  ctx.fillText(`${g.kills%KILLS_PER_WAVE}/${KILLS_PER_WAVE}`,W-12,75);

  // Multiplier bar
  let barY = 79;
  if (g.mult > 1) {
    const rem = Math.max(0,(g.multEnd-now)/9000);
    ctx.fillStyle="rgba(0,120,255,.1)"; rRect(ctx,8,barY,W-16,15,5); ctx.fill();
    const mg=ctx.createLinearGradient(8,0,8+(W-16)*rem,0);mg.addColorStop(0,"#0052ff");mg.addColorStop(1,"#00ccff");
    ctx.fillStyle=mg; rRect(ctx,8,barY,(W-16)*rem,15,5); ctx.fill();
    ctx.font="bold 8px 'Exo 2'";ctx.textAlign="center";ctx.fillStyle="#fff";ctx.fillText("$BASE x2 MULTIPLIER ACTIVE",W/2,barY+10);
    barY+=18;
  }
  // Freeze bar
  if (g.freezeEnd > now) {
    const rem=Math.max(0,(g.freezeEnd-now)/5000);
    ctx.fillStyle="rgba(0,200,195,.1)"; rRect(ctx,8,barY,W-16,15,5); ctx.fill();
    const fg=ctx.createLinearGradient(8,0,8+(W-16)*rem,0);fg.addColorStop(0,"#00ccbb");fg.addColorStop(1,"#aaffee");
    ctx.fillStyle=fg; rRect(ctx,8,barY,(W-16)*rem,15,5); ctx.fill();
    ctx.font="bold 8px 'Exo 2'";ctx.textAlign="center";ctx.fillStyle="#aaffee";ctx.fillText("ENEMIES FROZEN",W/2,barY+10);
    barY+=18;
  }
  // Laser active bar
  if (g.laserEnd > now) {
    const rem=Math.max(0,(g.laserEnd-now)/LASER_DURATION);
    ctx.fillStyle="rgba(200,50,0,.12)"; rRect(ctx,8,barY,W-16,15,5); ctx.fill();
    const lg=ctx.createLinearGradient(8,0,8+(W-16)*rem,0);lg.addColorStop(0,"#ff2200");lg.addColorStop(1,"#ffaa00");
    ctx.fillStyle=lg; rRect(ctx,8,barY,(W-16)*rem,15,5); ctx.fill();
    ctx.font="bold 8px 'Exo 2'";ctx.textAlign="center";ctx.fillStyle="#ffcc88";ctx.fillText("LASER BEAM ACTIVE",W/2,barY+10);
  }
}
...
export default function App() {
  ...
  // Auto-connect from Farcaster / Coinbase Wallet SDK on mount
  useEffect(() => {
    async function tryAutoConnect() {
      ...
      try {
        ...
        const addr = ctx?.user?.verifiedAddresses?.ethAddresses?.[0];
        if (addr) {
          setWallet(addr);
          const bn = await resolveBasename(addr);
          setBasename(bn || "");
          getBest(addr).then(setBest);
        }
      } catch {}
    }
    tryAutoConnect();
  ...
  const handleOver = useCallback(async (score) => {
    ...
    if (wallet) { await saveBest(wallet, score, basename); setBest(await getBest(wallet)); }
    setLb(await getLeaderboard()); setScreen("over");
  }, [wallet, basename]);

  const toggleMusic = () => { const on = audio.toggleMusic(); setMusicOn(on); };
  useEffect(() => { if (screen === "board") getLeaderboard().then(setLb); }, [screen]);
    ...
    <div style={{minHeight:"100vh",background:"#010108",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Exo 2','Courier New',sans-serif",padding:8}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes laser-pulse{0%,100%{opacity:.7}50%{opacity:1}}
        ...
        .btn-ic{background:rgba(0,20,65,.55);backdrop-filter:blur(6px);border:1px solid rgba(0,90,210,.25);border-radius:10px;color:rgba(130,195,255,.85);font-size:16px;padding:7px 11px;cursor:pointer;transition:all .18s;line-height:1;font-family:'Exo 2',sans-serif;font-weight:700}
      ...
      `}</style>
      {screen==="home"&&<HomeScreen inputVal={inputVal} setInputVal={setInputVal} wallet={wallet} basename={basename} connect={connect} connecting={connecting} best={best} setScreen={setScreen} musicOn={musicOn} toggleMusic={toggleMusic}/>}
...
/* ═══════════════════════════════════════════════════════════
   HOME SCREEN
═══════════════════════════════════════════════════════════ */
function HomeScreen({inputVal,setInputVal,wallet,basename,connect,connecting,best,setScreen,musicOn,toggleMusic}){
  const cvs=useRef(),raf=useRef(),ph=useRef(0),sc=useRef(0),stars=useRef(genStars());
  useEffect(()=>{
    const c=cvs.current;if(!c)return;
    const ctx=c.getContext("2d");
    const loop=()=>{ph.current+=.014;sc.current+=.38;ctx.fillStyle="#010108";ctx.fillRect(0,0,W,H);[[W*.75,H*.22,260,"rgba(0,32,110,.16)"],[W*.15,H*.72,200,"rgba(50,0,90,.1)"]].forEach(([nx,ny,nr,nc])=>{const ng=ctx.createRadialGradient(nx,ny,0,nx,ny,nr);ng.addColorStop(0,nc);ng.addColorStop(1,"transparent");ctx.fillStyle=ng;ctx.fillRect(0,0,W,H);});drawStars(ctx,stars.current,sc.current,ph.current);ctx.save();ctx.globalAlpha=.88;drawShip(ctx,W/2-SHIP_W/2,200+Math.sin(ph.current*.75)*10,false,false,false,ph.current*2.6);ctx.restore();raf.current=requestAnimationFrame(loop);};
    ...
    <div className="sc" style={{position:"relative",width:W,borderRadius:22,overflow:"hidden",boxShadow:"0 0 100px rgba(0,50,190,.32),0 0 0 1px rgba(0,80,220,.15)"}}>
      ...
      <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",padding:"26px 26px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:4}}>
          ...
          <div style={{fontSize:42,fontWeight:900,fontFamily:"'Rajdhani',sans-serif",letterSpacing:2,lineHeight:1,background:"linear-gradient(130deg,#fff 0%,#99d4ff 30%,#0099ff 60%,#0041dd 100%)",backgroundSize:"300% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"shimmer 5s linear infinite"}}>BASE<br/>INVADERS</div>
          <div style={{marginTop:10,display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
            ...
            <span style={{fontSize:8,color:"rgba(100,165,255,.4)",letterSpacing:5,fontFamily:"'Rajdhani',sans-serif",fontWeight:600,whiteSpace:"nowrap"}}>DEFENDER PROTOCOL v4.0</span>
        ...
        <div style={{background:"rgba(0,10,40,.75)",backdropFilter:"blur(16px)",border:"1px solid rgba(0,80,200,.2)",borderRadius:16,padding:"14px 18px",marginBottom:14}}>
          <div style={{fontSize:9,letterSpacing:4,color:"rgba(0,140,255,.5)",marginBottom:10,fontFamily:"'Rajdhani',sans-serif",fontWeight:700}}>CONTROLS & RULES</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}>
            {[["DRAG / ←→","Move ship"],["AUTO-FIRE","No trigger needed"],["🔵 $BASE","×2 score (rare)"],["❄ FREEZE","Stops enemies (rarer)"],["⚡ LASER","Double-tap to fire (store 1)"],["×10 KILLS","Wave difficulty +1"]].map(([k,v])=>(
              <div key={k} style={{display:"flex",gap:7,alignItems:"center"}}>
                <span style={{background:"rgba(0,60,190,.28)",border:"1px solid rgba(0,100,220,.25)",borderRadius:6,padding:"2px 7px",fontSize:9,color:"#7fc4ff",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,whiteSpace:"nowrap"}}>{k}</span>
        ...
        {/* Wallet connect — wallet address only, no sign-in */}
        {!isConnected ? (
          <div style={{marginBottom:12}}>
            <div style={{fontSize:8,letterSpacing:2,color:"rgba(100,150,255,.5)",marginBottom:6,textAlign:"center",fontFamily:"'Rajdhani',sans-serif",lineHeight:1.6}}>
              PASTE YOUR BASE WALLET ADDRESS<br/>
              <span style={{color:"rgba(80,120,200,.4)",fontSize:7}}>NO SIGN-IN REQUIRED · READ-ONLY · SCORES TIED TO YOUR ADDRESS</span>
            </div>
            <input value={inputVal} onChange={e=>setInputVal(e.target.value)} placeholder="0x… Base wallet address"
              style={{width:"100%",background:"rgba(0,12,50,.85)",border:"1px solid rgba(0,80,200,.22)",borderRadius:11,padding:"12px 15px",color:"rgba(175,215,255,.9)",fontSize:12,fontFamily:"'Exo 2',sans-serif",marginBottom:9,letterSpacing:.4}}/>
            <button className="btn-p" onClick={connect} disabled={!inputVal.trim()||connecting}
              style={{opacity:!inputVal.trim()||connecting?.6:1}}>
              {connecting?"RESOLVING…":"⚡  Connect Base Wallet"}
            </button>
            <div style={{marginTop:8,textAlign:"center",fontSize:8,color:"rgba(80,120,200,.4)",fontFamily:"'Exo 2',sans-serif",letterSpacing:1}}>
              Your basename will appear on the leaderboard automatically
            </div>
          </div>
        ) : (
          <div style={{background:"rgba(0,22,65,.65)",border:"1px solid rgba(0,120,255,.2)",borderRadius:12,padding:"11px 15px",marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:9}}>
                <div style={{width:9,height:9,borderRadius:"50%",background:"#00ff99",boxShadow:"0 0 10px #00ff99",animation:"breathe 2s ease-in-out infinite"}}/>
                <div>
                  <div style={{color:"#00ff99",fontSize:13,fontFamily:"'Rajdhani',sans-serif",fontWeight:700,letterSpacing:1}}>{basename || shortAddr(wallet)}</div>
                  {basename&&<div style={{color:"rgba(100,150,200,.5)",fontSize:9,fontFamily:"'Exo 2',sans-serif",letterSpacing:.5}}>{shortAddr(wallet)}</div>}
                </div>
              </div>
              {best>0&&<span style={{color:"#ffd700",fontSize:11,fontFamily:"'Rajdhani',sans-serif",fontWeight:700,letterSpacing:1}}>BEST {best.toLocaleString()}</span>}
            </div>
          </div>
        )}

        <button className="btn-p" style={{marginBottom:9,fontSize:17,padding:"18px",letterSpacing:4,animation:"glow-p 2.5s ease-in-out infinite"}} onClick={()=>setScreen("game")}>▶  LAUNCH</button>
...
function GameScreen({wallet,basename,onGameOver,musicOn,toggleMusic}){
  ...
  const prevWave=useRef(1),tiltRef=useRef(0);
  const pausedRef=useRef(false);
  const lastTap=useRef(0);
  const [paused,setPaused]=useState(false);
  const [waveMsg,setWaveMsg]=useState(null);
  const waveMsgTimer=useRef(null);
  const [score,setScore]=useState(0); // React state for overlay
  ...
  // Double-tap / double-click to fire laser
  const handleDoubleTap=useCallback(()=>{
    const g=gs.current;if(!g||g.dead||pausedRef.current)return;
    if(g.laserStored&&g.laserEnd<=Date.now()){
      g.laserStored=false;g.laserEnd=Date.now()+LASER_DURATION;
      audio.laserStart();
    }
  },[]);
  ...
  useEffect(()=>{
    ...
    const gk=e=>{
      ...
      if(e.type==="keydown"&&e.code==="Escape")togglePause();
    };
    ...
    const onS=e=>{
      ...
      drag.current={on:true,sx:getCX(e),ship0:g.tx};
  ...
  const loop=useCallback(ts=>{
    ...
    // Auto-fire bullets (only when no laser)
    ...
    // Wave progression
    if(g.kills>0&&g.kills%KILLS_PER_WAVE===0&&g.wave!==Math.floor(g.kills/KILLS_PER_WAVE)+1)g.wave=Math.floor(g.kills/KILLS_PER_WAVE)+1;
    if(g.wave!==prevWave.current){prevWave.current=g.wave;audio.waveUp();setWaveMsg(`WAVE ${g.wave}`);if(waveMsgTimer.current)clearTimeout(waveMsgTimer.current);waveMsgTimer.current=setTimeout(()=>setWaveMsg(null),2200);}

    // Spawn enemies
    const eInt=Math.max(340,1700-g.wave*75);
    if(now-lastEnemy.current>eInt){lastEnemy.current=now;const cnt=g.wave>6?3:g.wave>3?2:1;for(let i=0;i<cnt;i++){const bs=50+g.wave*15;g.enemies.push({x:rand(12,W-50),y:-52,speed:bs+rand(0,bs*.3),type:Math.floor(rand(0,3)),active:true,phase:rand(0,Math.PI*2)});}}
    // $BASE fruit
    ...
    // Laser fruit (rarest — 25-40s, only if not already stored)
    ...
    // Laser beam kills ALL enemies in column
    if(laserActive){
      const cx=g.px+SHIP_W/2;
      g.enemies.forEach(e=>{
        if(!e.active)return;
        if(cx>e.x-4&&cx<e.x+40){
          e.active=false;const pal=PALETTES[(g.wave-1)%PALETTES.length];
          g.score+=(20*g.mult*(frozen?2:1));g.kills++;
          burst(g,e.x+18,e.y+18,pal[0],16);audio.explode();
        }
      });
    }
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,position:"relative"}}>
      ...
      <div style={{position:"relative"}}>
        ...
        {/* Control buttons overlay — top right of canvas */}
        ...
        {/* Pause overlay */}
        {paused&&(
          <div style={{position:"absolute",inset:0,background:"rgba(0,3,20,.75)",borderRadius:18,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:5,backdropFilter:"blur(4px)"}}>
            <div style={{background:"rgba(0,10,40,.95)",border:"1px solid rgba(0,100,220,.35)",borderRadius:18,padding:"36px 52px",textAlign:"center"}}>
              ...
              <div style={{fontSize:54,fontWeight:900,color:"#fff",fontFamily:"'Rajdhani',sans-serif",lineHeight:1,marginBottom:20,letterSpacing:6}}>II</div>
              <div style={{fontSize:22,fontWeight:900,fontFamily:"'Rajdhani',sans-serif",color:"#ffd700",marginBottom:16}}>{score.toLocaleString()}</div>
              <button className="btn-p" style={{fontSize:14,padding:"13px 36px",letterSpacing:3,width:"auto"}} onClick={togglePause}>▶  RESUME</button>
              <div style={{marginTop:10,fontSize:8,color:"rgba(100,160,255,.4)",fontFamily:"'Exo 2',sans-serif",letterSpacing:1}}>ESC to toggle</div>
            </div>
          </div>
        )}
      </div>
      <div style={{color:"rgba(70,110,200,.48)",fontSize:9,letterSpacing:3,fontFamily:"'Rajdhani',sans-serif",textAlign:"center"}}>
        DRAG ← → MOVE  ·  AUTO-FIRE  ·  DOUBLE-TAP = LASER  ·  ESC PAUSE
      </div>
    </div>
  );
}
...
function GameOverScreen({score,best,wallet,basename,setScreen,musicOn,toggleMusic}){
  ...
  return(
    <div className="sc" style={{width:W,background:"rgba(1,1,16,.98)",border:"1px solid rgba(0,60,180,.2)",borderRadius:22,overflow:"hidden",boxShadow:"0 0 100px rgba(0,30,140,.42)"}}>
      <div style={{position:"absolute",top:-100,left:"50%",transform:"translateX(-50%)",width:400,height:400,borderRadius:"50%",background:"radial-gradient(circle,rgba(0,36,130,.2),transparent 70%)",pointerEvents:"none"}}/>
      <div style={{position:"relative",zIndex:1,padding:"48px 32px",textAlign:"center"}}>
        ...
        <div style={{background:"rgba(0,12,48,.7)",border:"1px solid rgba(0,60,180,.2)",borderRadius:18,padding:"30px 24px",margin:"0 0 28px"}}>
          ...
          {wallet&&<div style={{marginTop:8,fontSize:10,color:"rgba(0,200,100,.5)",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,letterSpacing:1}}>{basename||shortAddr(wallet)} · SAVED</div>}
        </div>
...
/* ═══════════════════════════════════════════════════════════
   LEADERBOARD — ranked by score, basename displayed
═══════════════════════════════════════════════════════════ */
    ...
    <div className="sc" style={{width:W,background:"rgba(1,1,16,.98)",border:"1px solid rgba(0,60,180,.2)",borderRadius:22,overflow:"hidden",boxShadow:"0 0 100px rgba(0,30,140,.42)"}}>
      <div style={{padding:"36px 28px"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{fontSize:9,letterSpacing:6,color:"rgba(255,195,0,.45)",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,marginBottom:5}}>GLOBAL RANKINGS</div>
          ...
          <div style={{marginTop:8,fontSize:8,color:"rgba(120,160,200,.4)",fontFamily:"'Exo 2',sans-serif",letterSpacing:2}}>Ranked by highest score · Basenames shown</div>
        </div>
        {lb.length===0?(
          <div style={{textAlign:"center",color:"rgba(80,120,180,.44)",padding:"48px 0",fontSize:12,fontFamily:"'Rajdhani',sans-serif",letterSpacing:3,lineHeight:2}}>NO SCORES RECORDED<br/>BE THE FIRST DEFENDER</div>
        ):(
          <div style={{display:"flex",flexDirection:"column",gap:7,maxHeight:460,overflowY:"auto",paddingRight:2}}>
            {lb.map((e,i)=>{
              ...
              return(
                <div key={e.wallet} style={{display:"flex",alignItems:"center",gap:12,
                  background:isMe?"rgba(0,50,165,.25)":(rankBg[i]||"rgba(0,14,45,.5)"),
                  border:`1px solid ${isMe?"rgba(0,120,255,.42)":i===0?"rgba(255,200,0,.22)":i===1?"rgba(200,200,200,.12)":i===2?"rgba(180,120,50,.12)":"rgba(0,50,140,.15)"}`,
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{color:isMe?"#7fc4ff":i===0?"#ffd700":"rgba(175,215,255,.88)",fontSize:13,fontWeight:800,fontFamily:"'Rajdhani',sans-serif",letterSpacing:.8,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                      {name}
                      {isMe&&<span style={{marginLeft:8,fontSize:8,color:"#0099ff",letterSpacing:2,background:"rgba(0,70,200,.3)",padding:"1px 7px",borderRadius:4}}>YOU</span>}
                    </div>
                    {e.basename&&<div style={{color:"rgba(100,140,200,.4)",fontSize:9,fontFamily:"'Exo 2',sans-serif",letterSpacing:.5,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginTop:1}}>{shortAddr(e.wallet)}</div>}
                  </div>
                  {/* Score */}
